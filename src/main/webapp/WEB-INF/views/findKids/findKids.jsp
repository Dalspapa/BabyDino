			...
			<div class="search">
			 	<form id="searchForm" class="" action="">
					...
					<section>
						...
						<hr>
						<div class="row" id="selectDateRow">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#calendar').pignoseCalendar({
				...
				format : 'YYYY-MM-DD',
				select: function(r) {
					$("input[name=reserve_date]").val(r[0]._i);
// 					if(selectDate == null) {
...
// 					}
		...
		function fnShowTeacherList() {
			...
			if( typeof params.k_care_type_list == 'string' ){
				var careType = params.k_care_type_list;
				params.k_care_type_list = [];
				params.k_care_type_list.push(careType);
			}

			//연령 리스트 각연령을 배열로 담아준다.
			if( typeof params.age_list == 'string' ){
				params.age_list = params.age_list.split(',');
			}
			else if( params.age_list instanceof Array ){
				var tempAgeList = [];
				for(var i in params.age_list){
					tempAgeList = tempAgeList.concat(params.age_list[i].split(','));
				}
				params.age_list = tempAgeList;
			}
			//validation 추가
