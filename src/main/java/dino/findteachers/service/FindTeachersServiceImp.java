...
public class FindTeachersServiceImp implements FindTeachersService {
	...
	public void makeKCard(KidDto dto, Common_ImgDto imgDto, List<MultipartFile> imgFiles, String dirPath) {
		...
		for (int i = 0; i < imgFiles.size(); i++) {
			...
			System.out.println(">>copy 성공입니다람쥐~<<");
			c_imgpath += imgFiles.get(i).getOriginalFilename() + ";";
		}
		System.out.println(">>이미지 없음 ?<<");
		System.out.println(">>파일이름은 ?<< = " + c_imgpath);

		int result = findTeachersDao.makeKCard(dto);

		if (result == 0) {
			System.out.println("insert 에러이빈다~");
		}
		...
		// test code
		System.out.println("이미지 DTO 값 : " + imgDto.getD_member_idx() + "/" + imgDto.getC_imgpath() + "/"
				+ imgDto.getCategory_idx() + "/" + imgDto.getRef_idx());
	...
	private void copyInto(MultipartFile imgFiles) {
		try {
	...
	public int kSetImg(Common_ImgDto imgDto) {
		...
		// test code
		System.out.println("findteacherservice setTimg imgpath" + imgDto.getC_imgpath() + "memberidx"
				+ imgDto.getD_member_idx() + "ref idx" + imgDto.getRef_idx());
		return result;
	}
	...
	/*
	 * // make kid card public int makeKCard(KidDto dto) {
	 * 
	 * int result = findTeachersDao.makeKCard(dto); return result; }
	 */
